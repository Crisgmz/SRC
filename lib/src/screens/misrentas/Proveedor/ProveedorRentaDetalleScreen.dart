...
class ProveedorRentaDetalleScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Detalle de Renta')),
      body: FutureBuilder<DocumentSnapshot>(
        future:
            FirebaseFirestore.instance.collection('rentas').doc(rentaId).get(),
          ...
          return Padding(
            ...
            child: ListView(
              children: [
                ...
                const SizedBox(height: 8),
                Text(
                  'Desde ${DateFormat('dd/MM/yyyy HH:mm').format(fechaInicio)}',
                ),
                Text(
                  'Hasta ${DateFormat('dd/MM/yyyy HH:mm').format(fechaFin)}',
                ),
